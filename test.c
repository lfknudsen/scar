...
int main (int argc, char** argv) {
    if (system("test -d tests"))            system("mkdir tests");
    if (system("test -d tests/out"))        system("mkdir tests/out");
    if (system("test -d tests/exp"))        system("mkdir tests/exp");
    if (system("test -d tests/programs"))   system("mkdir tests/programs");
    int skip_run = 0;
    if (argc >= 2 && strcmp(argv[1], "-s") == 0) {
        skip_run = 1;
        printf("Skipping interpreting. Just printing previous results.\n");
    }
    const char* base_exec     = "./scar -q ";
    const char* test_option   = " > ";
    const char* new_extension = ".out";
    const char* base_comp     = "cmp -s ";
    ...
    while (i < n) {
        if (sorted_programs[i]->d_type == DT_REG) {
            ...
            if (skip_run == 0) {
                exec_cmd = malloc(base_exec_len + strlen(sorted_programs[i]->d_name) * 2 + 4);
                sprintf(exec_cmd, "%s %s%s %s %s%s%s", base_exec, prog_dirname, name, test_option, out_dirname, name, out_ext);
                system(exec_cmd);
                free(exec_cmd);
            }
            comp_cmd = malloc(base_comp_len + strlen(name) * 2 + 1);
